...
describe 'generate article' do
  ...
  let(:cli) do
    Adjutant::CommandLineInterface.new(['exercise']).tap do |c|
  ...
  let(:md_path) { "exercise/#{slug}/#{slug}.md" }

  it 'creates a new directory' do
    run
    expect(has_dir?("exercise/#{slug}")).to be(true)
  end

  it 'creates a .lesson.yml' do
    run
    expect(has_file?("exercise/#{slug}/.lesson.yml")).to be(true)
  end

  it 'creates a .gitignore' do
    run
    expect(has_file?("exercise/#{slug}/.gitignore")).to be(true)
  end
